...
class TroubleCreate(TroubleBase):
    ...
    user_query: str = Field(
        ..., description="사용자의 자연어 질의", min_length=1, max_length=1000
    )
    related_logs: List[str] = Field(
        ..., description="트러블슈팅과 연관된 로그 id들", min_items=1
    )
...
class TroubleWithLogs(BaseModel):
    """Trouble 응답용 스키마 - 연관된 로그들을 포함"""
...
class TroubleListQuery(BaseModel):
    ...
    search: Optional[str] = Field(
        None, description="검색어 (report_name, user_query 대상)"
    )
